    ...
    async signInWithGoogle() {
        return new Promise((resolve, reject) => {
            try {
                ...
                // Use Chrome Identity API for real OAuth2 with account selection
                chrome.identity.getAuthToken({
                    ...
                    if (chrome.runtime.lastError) {
                        ...
                        console.error('🔧 Fix: Update Google Cloud Console OAuth2 client with Extension ID:', extensionId);
                        
                        // Handle specific OAuth configuration errors
                        if (errorMessage.includes('client_id') || errorMessage.includes('OAuth2') || errorMessage.includes('bad client id')) {
                            console.log('⏳ OAuth2 config propagating, using temporary auth...');
                            // Temporary fallback while Google propagates OAuth2 changes
                            const mockUser = {
                                id: 'temp-user-' + Date.now(),
                                email: 'user@beatschain.com',
                                name: 'BeatsChain User',
