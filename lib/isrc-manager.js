...
class ISRCManager {
    ...
    async loadISRCRegistry() {
        try {
            ...
            if (this.registry.year !== this.currentYear) {
                this.registry.lastDesignation = 0;
    ...
    enhanceRadioForm() {
        ...
        generateBtn.className = 'btn btn-secondary btn-sm isrc-generate-btn';
        generateBtn.innerHTML = '🎵 Generate ISRC';
        generateBtn.title = 'Generate professional ISRC code (80G registrant)';
        
        generateBtn.addEventListener('click', () => {
            this.handleISRCGeneration();
        });

        // Insert button after ISRC field
        const formRow = isrcField.closest('.form-row');
        if (formRow) {
            const buttonContainer = document.createElement('div');
            buttonContainer.className = 'isrc-controls';
