...
class ThirdwebManager {
    ...
    async mintNFT(recipientAddress, metadataUri) {
        try {
            ...
            // Use Thirdweb Engine API for proper SDK integration
            const response = await fetch(`https://api.thirdweb.com/v1/chains/80001/contracts/${this.contractAddress}/mint`, {
                method: 'POST',
                headers: {
            ...
            if (response.ok) {
                const result = await response.json();
                console.log('✅ Thirdweb SDK mint successful:', result);
                ...
                console.warn('Thirdweb API failed, falling back to direct RPC');
                throw new Error('Thirdweb API failed');
            }
        } catch (error) {
    ...
    async mintViaDirectRPC(recipientAddress, metadataUri) {
        try {
            ...
            // Fallback to testnet simulation with clear marking
            console.log('🔄 Falling back to testnet simulation...');
    ...
    async createTestnetTransaction(recipientAddress, metadataUri) {
        ...
        const dataToHash = `TESTNET_${recipientAddress}${metadataUri}${timestamp}${nonce}`;
        ...
        console.log('⚠️ TESTNET SIMULATION - NOT REAL BLOCKCHAIN');
        console.log('📍 Simulated Hash:', txHash);
